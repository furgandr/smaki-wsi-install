...
If you want to spin up a disposable server locally, the repository already ships with a Vagrant definition plus inventory entries.

1. Install the Python toolchain (pyenv + pyenv-virtualenv) and bootstrap dependencies via `./bin/pyenv-install`.
2. Fetch the required Ansible Galaxy roles with `bin/setup`.
3. Start the VM with `vagrant up`. The inventory entry is `192.168.56.4`, but it connects through the forwarded port on `127.0.0.1:2222` using Vagrant’s default key.
4. Provision + deploy Open Food Network by running `ansible-playbook site.yml --limit=192.168.56.4 -u vagrant`. The limit matches the host defined in `inventory/hosts` and auto-loads the host vars from `inventory/host_vars/192.168.56.4/config.yml`.
5. Once the playbook finishes, open `http://localhost:8080` (or `https://192.168.56.4.nip.io`) to reach the app with the default credentials defined in `inventory/group_vars/pl.yml`.

Feel free to tweak the host-specific settings in `inventory/host_vars/192.168.56.4/config.yml` if you need a different domain, locale, or integration options.
